...
DEMO_EMAIL="$(grep '^DEFAULT_USER_EMAIL=' "${ENV_FILE}" | cut -d= -f2-)"
DEMO_PASSWORD="$(grep '^DEFAULT_USER_PASSWORD=' "${ENV_FILE}" | cut -d= -f2-)"
...
for _ in $(seq 1 60); do
  if curl -fsS "http://127.0.0.1:7080/gateway/health" >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
...
bash "${ROOT_DIR}/scripts/smoke_test.sh" --base-url "http://127.0.0.1:7080"
